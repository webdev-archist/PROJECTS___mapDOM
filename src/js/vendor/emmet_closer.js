...
let emmet = () => {
    ...
    function emmet(text, htmlOnly, args) {
        ...
        usedText
            ...
            .replace(indexesRe, function (full, elementText, splitter) {
                ...
                else if (splitter.indexOf("^") != -1)
                    splitter.split('').forEach(nth => {current = current.parentNode})
                else if (splitter.indexOf('*') != -1) current = lastElement
            });
    ...
    if (window.jQuery) {
        window.jQuery.$$$ = function (text, htmlOnly, args) {
            var el = emmet(text, htmlOnly, args);
            return htmlOnly ? el : window.jQuery(el);
        };
        window.jQuery.$$$.template = function (text, htmlOnly, args) {
